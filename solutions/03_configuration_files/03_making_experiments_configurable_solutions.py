...
# %%
# Solution
cfg = {"conditions": ["a", "b", "c"], "probability": [0.3, 0.2, 0.6], "n_blocks":3}
with open("config.json", "w") as f:  # write again so the notebook runs
    json.dump(cfg, f, indent=4)
...
# %%
# Solution
cfg = {"height": 1, "width": 1, "color":"white"}
with open("rect_config.json", "w") as f:
    json.dump(cfg, f, indent=4)
...
# %%
# Solution
with open("rect_config.json", "r") as f:
    cfg = json.load(f)
...
# %%
# Solution
x = {"pos":(0, 0.5), "color":"red", "keys":["x", "m"]}
with open("circle_config.json", "w") as f:
...
# %%
# Solution
with open("circle_config.json", "r") as f:
    cfg = json.load(f)
...
# %%
# Solution
x = {"width":2, "height":2, "color":"teal", "text":"Welcome!"}
with open("text_config.json", "w") as f:
    json.dump(x, f, indent=4)
...
# %%
# Solution
with open("text_config.json") as f:
    cfg = json.load(f)
...
# Solution
with Window() as win:
    rect = Rect(win, height=cfg["height"], width=cfg["width"], fillColor=cfg["color"])
    text = TextStim(win, text=cfg["text"])
    rect.draw()
    text.draw()
    win.flip()
    waitKeys()
...
# %%
# Solution
x = {"rect":{"width":1, "height":1, "color":"orange"}, "circle":{"color":"magenta", "radius":0.1}}
with open("visual_config.json", "w") as f:
    json.dump(x, f, indent=4)
...
# %%
# Solution
with open("visual_config.json") as f:
    cfg = json.load(f)
...
# Solution
with Window() as win:
    rect = Rect(win, height=cfg["rect"]["height"], width=cfg["rect"]["width"], fillColor=cfg["rect"]["color"])
    rect.draw()
    win.flip()
    waitKeys()
    circle = Circle(win, radius=cfg["circle"]["radius"], fillColor=cfg["circle"]["color"])
    circle.draw()
    win.flip()
    waitKeys()
